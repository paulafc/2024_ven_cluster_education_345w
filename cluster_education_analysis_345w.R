...
pacman::p_load(tidyverse, readxl, openxlsx, writexl, sf, leaflet, fuzzyjoin, dplyr, purrr)
...
indicators<-indicators %>% 
  mutate(number_activity = sub("([^:]*).*", "\\1", sub_activity))
...
target.adm0 <- calculate_sum(data = temp %>% mutate('#adm0+name'='Venezuela', '#adm0+code'='VEN0'),activity = 'none',aggregation = c('#adm0+name','#adm0+code'),targets_summarise = c(col.totals,col.soma,col.reached))
...
df_list <- list(
  ...
  'adm1_target' = t.adm1,
  'adm1__ind_obj' = df.adm1,
  
  'adm2_target' = t.adm2,
  'adm2__ind_obj' = df.adm2)
